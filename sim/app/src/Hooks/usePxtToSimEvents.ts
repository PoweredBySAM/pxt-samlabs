...
function usePxtToSimEvents(device: SamDeviceStoreType) {
  ...
  const callback = (event: CustomEvent) => {
    ...
    singleDeviceStore.setDeviceProp(
  ...
  const addPxtEvents = () => {
    CustomEventGenerator.getInstance().receiveEvent(
      `${BasicEvents.TOSIM_DEVICE_VALUE_CHANGED}_${singleDeviceStore._deviceId}`,callback);
  };
  const removePxtEvents = () =>{
    CustomEventGenerator.getInstance().unregisterEvent(`${BasicEvents.TOSIM_DEVICE_VALUE_CHANGED}_${singleDeviceStore._deviceId}`,callback)
  }
