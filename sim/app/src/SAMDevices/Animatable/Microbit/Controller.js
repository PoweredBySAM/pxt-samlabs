...
class Controller extends eventEmitter {
    ...
    connect = (callback = () => {}) => {
        ...
        navigator.bluetooth.requestDevice({
            ...
            .then((services) => {
                this._services.uartService = services.find(a => a.uuid === '6e400001-b5a3-f393-e0a9-e50e24dcca9e')
                this._services.ledService = services.find(a => a.uuid === 'e95dd91d-251d-470a-a062-fa1922dfa9a8')
                this._services.buttonService = services.find(a => a.uuid === 'e95d9882-251d-470a-a062-fa1922dfa9a8')
                this._services.temperatureService = services.find(a => a.uuid === 'e95d6100-251d-470a-a062-fa1922dfa9a8')
                this._services.accelerometerService = services.find(a => a.uuid === 'e95d0753-251d-470a-a062-fa1922dfa9a8')

                if(!this._services.uartService) throw new Error('Invalid file flashed to microbit')
            })
            .then(() => this._services.uartService.getCharacteristics())
            .then((uartCharacteristics) => {
                this._characteristics.uartRead = uartCharacteristics.find(a => a.uuid === '6e400002-b5a3-f393-e0a9-e50e24dcca9e')
                this._characteristics.uartWrite = uartCharacteristics.find(a => a.uuid === '6e400003-b5a3-f393-e0a9-e50e24dcca9e')
                this._characteristics.uartRead.oncharacteristicvaluechanged = this._onUart
            ...
            .then((ledCharacteristics) => {
                this._characteristics.ledText = ledCharacteristics.find(a => a.uuid === 'e95d93ee-251d-470a-a062-fa1922dfa9a8')
                this._characteristics.ledMatrix = ledCharacteristics.find(a => a.uuid === 'e95d7b77-251d-470a-a062-fa1922dfa9a8')
            })
            .then(() => this._services.buttonService.getCharacteristics())
            .then((buttonCharacteristics) => {
                this._characteristics.buttonA = buttonCharacteristics.find(a => a.uuid === 'e95dda90-251d-470a-a062-fa1922dfa9a8')
                this._characteristics.buttonB = buttonCharacteristics.find(a => a.uuid === 'e95dda91-251d-470a-a062-fa1922dfa9a8')
                this._characteristics.buttonA.oncharacteristicvaluechanged = this._onButtonAChange
                this._characteristics.buttonB.oncharacteristicvaluechanged = this._onButtonBChange
            })
            ...
            .then((temperatureCharacteristics) => {
                this._characteristics.temperaturePeriod = temperatureCharacteristics.find(a => a.uuid === 'e95d1b25-251d-470a-a062-fa1922dfa9a8')
                this._characteristics.temperatureRead = temperatureCharacteristics.find(a => a.uuid === 'e95d9250-251d-470a-a062-fa1922dfa9a8')
                this._characteristics.temperatureRead.oncharacteristicvaluechanged = this._onTemperatureChange
            })
            .then(() => this._characteristics.temperaturePeriod.writeValue(new Uint8Array([160, 0])))
            ...
            .then((accelerometerCharacteristics) => {
                this._characteristics.accelerometerPeriod = accelerometerCharacteristics.find(a => a.uuid === 'e95dfb24-251d-470a-a062-fa1922dfa9a8')
                this._characteristics.accelerometerRead = accelerometerCharacteristics.find(a => a.uuid === 'e95dca4b-251d-470a-a062-fa1922dfa9a8')
                this._characteristics.accelerometerRead.oncharacteristicvaluechanged = this._onAccelerometerChange
            })
            // https://github.com/lancaster-university/microbit-dal/issues/445
            .then(() => this._characteristics.accelerometerPeriod.writeValue(new Uint8Array([100, 0])))
            .then(() => this._characteristics.accelerometerRead.startNotifications())
            .then(() => {
                this._connected = true
                this._connecting = false
                this._device.addEventListener('gattserverdisconnected', this.disconnect)
                this.emit('connected')
                callback()
            })
            .catch((err) => {
    ...
    disconnect = () => {
        if(this._device) {
            Object.keys(this._characteristics).forEach((c) => this._characteristics[c].oncharacteristicvaluechanged = null)
            this._device.removeEventListener('gattserverdisconnected', this.disconnect)
    ...
    _verifyFlash = () => {
        ...
        return Promise.race([
            ...
            new Promise((resolve, reject) => {
                var checkVerified = () => {
                    if(this._flashVerifiedState === FLASH_VERIFICATION_STATE.OUT_OF_DATE) {
                        failed = true
                    ...
                    else if(this._flashVerifiedState === FLASH_VERIFICATION_STATE.VERIFIED) return resolve()
    ...
    writeUart = (value) => (
        ...
        this._characteristics.uartWrite.writeValue(new Uint8Array(Array.from(new TextEncoder('utf8').encode(value))))
    )
    ...
    displayText = (text, callback) => {
        ...
        this._characteristics.ledText.writeValue(new Uint8Array(Array.from(new TextEncoder('utf8').encode(text))))
    ...
    plot = (x, y) => {
        ...
        if(x > 5 || x < 0 || y > 5 || y < 0) throw new Error('Invalid coordinates')
    ...
    unplot = (x, y) => {
        if(isNaN(x) || isNaN(y)) throw new Error('Invalid coordinates')
        if(x > 4 || x < 0 || y > 4 || y < 0) throw new Error('Invalid coordinates')
    ...
    _writeLEDMatrix = () => {
        ...
        this._characteristics.ledMatrix
            .writeValue(new Uint8Array([toValue(matrix[0]), toValue(matrix[1]), toValue(matrix[2]), toValue(matrix[3]), toValue(matrix[4])]))
            .then(() => {
                this._writingLEDMatrix = false
    ...
    _onUart = (event) => {
        var value = String.fromCharCode.apply(null, new Uint8Array(event.target.value.buffer))
        // TODO Remove this Workbench check later
